...
//    $taxonomyId = [];
//    foreach ($_REQUEST['group_material'] as $index => $item) {
//        $taxonomyId[] = $item['taxonomy_material'];
//    }
//    wp_set_object_terms( $post_id, $taxonomyId, 'material');
//    $log = new Logger('save_post');
//    $log->pushHandler(new StreamHandler(get_theme_file_path().'/logs.log', Logger::WARNING));
//    $log->warning($post_id.' = '.print_r($_REQUEST, true));
//});

function widget_posts_args_add_custom_type($params) {
    $params['post_type'] = array('post','cookbook');
    return $params;
}
...
function search_filter($query) {
    if ( ! is_admin() && $query->is_main_query() ) {
        if ( $query->is_search ) {
            $query->set( 'post_type', ['post', 'cookbook'] );
        }
    }
}
